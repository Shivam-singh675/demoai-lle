/*-----------------------------------------------------------------------------
    ...
    Flow details:
    Build details: 08/03/2026 01:46:30
...
void Flow_1()
{
    ...
    ns_web_url("index",
        "URL={BASE_URL}{post_setup_url}",
        "METHOD=POST",
        "HEADER=Authorization:{dynamic_auth_header}",
        BODY_BEGIN,
            "{"name":"{dynamic_post_name}"}",
        BODY_END
    );
    ...
    ns_web_url("index1",
        "URL={BASE_URL}/api/catalog-items/{dynamic_delete_catalogItemId}",
        "METHOD=DELETE",
        "HEADER=Authorization:{dynamic_auth_header}"
    );
