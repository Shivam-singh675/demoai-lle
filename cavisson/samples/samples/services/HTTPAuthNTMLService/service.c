...
char conn_break[128] = "N"; 
  ...
  if(!strcmp(cookie_val, ""))
  {
    ...
    if(!strcmp(test_case, "T0Fail"))
    {
      ns_set_http_hdr("WWW-Authenticate: Negotiate");
      ns_set_http_hdr("WWW-Authenticate: NTLM BAD");
    }
    else
    {
      ns_set_http_hdr("Set-Cookie: AuthMsg=1");
      ns_set_http_hdr("WWW-Authenticate: Negotiate");
      ns_set_http_hdr("WWW-Authenticate: NTLM");
      int ret = ns_get_url_query_param_val("ConnBreak", conn_break, HPD_CODE_NONE); 
      if (conn_break[0] == 'Y')    
      ns_ka_time(0);  
    }
  ...
  else if(!strcmp(cookie_val, "1"))
  {
    ...
    {
      ns_set_http_hdr("Set-Cookie: AuthMsg=2");
      ns_set_http_hdr("WWW-Authenticate: NTLM TlRMTVNTUAACAAAABAAEADAAAAAGggEAh8te86Jk7IYAAAAAAAAAAAAAAAA0AAAAdGVzdA==");
      ns_ka_time(45000);
    }
    fill_body(AUTH_FAILURE_RESPONSE2);
  }
  else if(!strcmp(cookie_val, "2"))
  {
    // Set cookie to empty so that we can handle new request on same connection
    ns_set_http_hdr("Set-Cookie: AuthMsg=");

    if(!strcmp(test_case, "T3Fail") || !strcmp(test_case, "T3FailNoNTLMHdr"))
    {
      ns_set_http_status_code(401);
      if(!strcmp(test_case, "T3Fail"))
        ns_set_http_hdr("WWW-Authenticate: NTLM");
      fill_body(AUTH_ERROR_RESPONSE);
    }
    else
    {
      if(!strcmp(test_case, "T3Redirect"))
      {
        ns_set_http_status_code(301);
        set_location();
      }
      else if(!strcmp(test_case, "T3Status"))
      {
        check_and_set_status_code();
      }
      else if(!strcmp(test_case, "T3Cacheable"))
      {
...
int HTTPAuthNTMLService_on_request_callback() 
{
