...
static void set_response()
{
  ...
  if(!strcmp(cookie_val, ""))
  {
    ...
    if(!strcmp(test_case, "MainReqFail"))
    {
      ...
      ns_set_http_hdr("WWW-Authenticate: Bad Kerberos ");
    } else{
      ns_set_http_hdr("Set-Cookie: AuthMsg=1");
      ns_set_http_hdr("WWW-Authenticate: Negotiate");
      ns_set_http_hdr("WWW-Authenticate: Basic realm=Kerberos Login");
    } 
  ...
  else if(!strcmp(cookie_val, "1"))
  {
	...
	if(!strcmp(test_case, "kerberosRealmFail"))   
    {
      ns_set_http_status_code(401);
      ns_set_http_hdr("WWW-Authenticate: Negotiate ");
      ns_set_http_hdr("WWW-Authenticate: Basic realm=Kerberos Login");
    ...
    {
      if(!strcmp(test_case, "kerberosRedirect"))
      {
        ns_set_http_status_code(301);
        set_location();
      } else if(!strcmp(test_case, "kerberosStatus")){
        check_and_set_status_code();
      }else if(!strcmp(test_case, "KerberosCacheable"))
      {
...
TestCase - 
    OK (Default is OK)
    MainReqFail - Send BASIC followed by junk
    BasicRealmFail - Send again 401
    BasicRedirect - Send 301 (redirect) and NO WWW-Authenticate header with Location header
    BasicStatus - Send status as per Status and NO WWW-Authenticate header. Usefule to simulate 404 or 500 on final reponse
    BasicCacheable - Send status 200 and make response cacheable
...
int HTTPAuthKerberosService_on_request_callback() 
{
